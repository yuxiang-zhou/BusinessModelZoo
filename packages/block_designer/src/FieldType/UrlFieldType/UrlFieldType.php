...
class UrlFieldType extends FieldType
{
    ...
    public function getValidateFunctionContents()
    {
        if ($this->getRepeating()) {
            ...
            $e->add(t("The %s field does not have a valid URL (%s, row #%s).", "' . h($this->data['label']) . '", "' . h($this->data['parent']['label']) . '", $' . $this->data['parent']['slug'] . '_k));
        }';
        } else {
            ...
            $e->add(t("The %s field does not have a valid URL.", "' . h($this->data['label']) . '"));
        }';
        }
    }
