        ...
        if ($this->getRepeating()) {
            ...
            if(isset($' . $this->data['parent']['slug'] . '_v[\'' . $this->data['slug'] . '\']) && trim($' . $this->data['parent']['slug'] . '_v[\'' . $this->data['slug'] . '\']) != ""){
                ...
                if ((!isset($matches[1]) || trim($matches[1]) == "")) {
                    $e->add(t("The %s field contains an invalid YouTube URL (%s, row #%s).", "' . h($this->data['label']) . '", "' . h($this->data['parent']['label']) . '", $' . $this->data['parent']['slug'] . '_k));
                }
            }
        }';
        } else {
            ...
            if ($args["' . $this->data['slug'] . '"] != "" && (!isset($matches[1]) || trim($matches[1]) == "")) {
                $e->add(t("The %s field contains an invalid YouTube URL.", "' . h($this->data['label']) . '"));
            }
        }';
        }
    }
