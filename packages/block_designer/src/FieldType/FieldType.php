...
class FieldType
{
    ...
    public function generateFormContent($type, $values = array(), $repeatable = false)
    {
        switch ($type) {
            case 'label':
                if ($repeatable) {
                    return '<label for="<?php  echo $view->field(\'' . $values['parent']['slug'] . '\'); ?>[{{id}}][' . $values['slug'] . ']"><?php  echo t("' . h($values['label']) . '")' . (isset($values['suffix']) ? $values['suffix'] : null) . '; ?></label>';
                } else {
                    return '<?php  echo $form->label(\'' . $values['slug'] . '\', t("' . h($values['label']) . '")' . (isset($values['suffix']) ? $values['suffix'] : null) . '); ?>';
                }
                break;
            case 'required':
                ...
                if ($repeatable) {
                    ...
                    switch ($type) {
                        case 'text':
                            ...
                            $baseAttributes['value'] = isset($values['value']) ? $values['value'] : '{{ ' . $values['slug'] . ' }}';
                            $attributes = isset($values['attributes']) && is_array($values['attributes']) && !empty($values['attributes']) ? array_merge($baseAttributes, $values['attributes']) : $baseAttributes;
