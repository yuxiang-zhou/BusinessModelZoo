...
if (isset($' . $slug . ') && !empty($' . $slug . ')){ ?>' . $this->data['prefix'] . '<?php  foreach($' . $slug . ' as $' . $slugStack . '){
    ...
    public function getViewFunctionContents()
    {
        ...
        if ($' . $this->data['slug'] . '_entries = $db->GetAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? AND eID = ? ORDER BY sortOrder ASC\', array($this->bID, $' . $this->data['parent']['slug'] . '_item_v[\'id\']))) {
        ...
        if ($' . $this->data['slug'] . '_entries = $db->GetAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID))) {
            foreach ($' . $this->data['slug'] . '_entries as $' . $this->data['slug'] . '_entry) {
    ...
    {
        return '$' . $this->data['slug'] . '_entries = $db->GetAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID));
        foreach ($' . $this->data['slug'] . '_entries as $' . $this->data['slug'] . '_entry) {
    ...
    public function getSaveFunctionContents()
    {
        ...
        $' . $slugQueries . ' = array();
        if ($' . $slugEntries . ' = $db->GetAll(\'SELECT * FROM ' . $entriesTableName . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID))) {
...
<script type="text/javascript">
    ...
    {
        if ($this->data['ft_count'] <= 0) {
            return '$al->register(\'javascript\', \'select2sortable\', \'blocks/' . $this->data['block_handle'] . '/js_form/select2.sortable.js\', array(), $this->pkg);';
        }
    }
