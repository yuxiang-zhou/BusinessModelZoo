...
class BlockDesigner extends DashboardPageController
{
                    ...
                    try {
                        ...
                        if ($btID > 0 && $btsID) {
                            ...
                            if (array_key_exists($btsID, $blockTypeSets)) {
                                $db = \Database::get();
                                $order = 0;
                                if ($displayOrder = $db->getRow('SELECT (MAX(displayOrder) + 1) AS displayOrder FROM BlockTypeSetBlockTypes WHERE btsID = ?', array($btsID))) {
                                    $order = (int)$displayOrder['displayOrder'];
                                }
                                $insert_values = array($btID, $btsID, $order);
                                $db->execute('INSERT INTO BlockTypeSetBlockTypes (btID, btsID, displayOrder) values(?,?,?)', $insert_values);
                            }
                        }
    ...
    public function on_start()
    {
        ...
        $al->register('javascript', 'handlebars', 'js/handlebars-v4.0.2.js', array(), $this->packageHandle);
